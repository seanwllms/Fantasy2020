---
title: "Fantasy 2016"
output: md_document 
---

```{r, echo=FALSE, warning=FALSE, error = FALSE, message = FALSE}
setwd("/home/sean/Documents/rstuff/fantasy2017")
source("compileleague.R")
library(dplyr)
library(purrr)
```

####Current Standings
```{r, echo = FALSE}
standings.output %>% mutate(hit.points = R_points + HR_points + RBI_points + SB_points + AVG_points,
                            pitch.points = ERA_points + WHIP_points + K_points + SV_points + W_points) %>%
                            select(team_name, spent, left, picks.left, total_points, hit.points,
                              pitch.points)

```

####Top Remaining Pitchers
```{r, echo = FALSE}
toppitch <- filter(pitcher_projections, status != "drafted") %>%
      mutate(pts = marginal.total.points, dlr = dollar.value) %>%
      select(name, Team, IP, ERA, WHIP, K, SV, W, pts, dlr)

toppitch[1:20,]
```

####Top Remaining Hitters
```{r, echo = FALSE}
round_zero_vars <- c("AB", "R", "HR", "RBI", "SB")
round_three_vars <- c("AVG",  "pts", "dlr")

topdh <- filter(hitter_projections, status != "drafted") %>%
      mutate(pts = marginal.total.points, dlr = dollar.value) %>%
      select(name, Team, AB, R, HR, RBI, SB, AVG, pts, dlr)  %>%
      arrange(-dlr) %>%
      map_at(round_zero_vars, round, 0) %>%
      map_at(round_three_vars, round, 3)

data.frame(topdh)[1:20,]
```

####Top Remaining OF
```{r, echo = FALSE}
topof <- filter(hitter_projections, status != "drafted") %>%
      mutate(pts = marginal.total.points, dlr = dollar.value) %>%
      select(name, Team, position, AB, R, HR, RBI, SB, AVG, pts, dlr) %>%
      filter(position=="outfield") %>%
      select(-position) %>%
      arrange(-dlr) %>%
      map_at(round_zero_vars, round, 0) %>%
      map_at(round_three_vars, round, 3)

data.frame(topof)[1:20,]
```

####Data entry errors
```{r, echo = FALSE}
drafterrors
```

```{r, echo=FALSE, warning=FALSE, error = FALSE, message = FALSE}
write.csv(pasadena, "pasadena.csv")
```